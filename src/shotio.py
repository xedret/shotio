...
class ShotIO( object ) :
 ...
 def buildCommentsList( self, client, asset_id, comment_list):
  """
  ...
  for asset in assets:
   ...
   if (asset.get('type') == 'folder') and (asset.get('item_count') > 0):
    self.buildCommentsList(client, asset['id'], comment_list)
 ...
 def flattenDict( self, d ):
  ...
  def expand(key, val):
   if isinstance(val, dict):
    return [ (key + '.' + k, v) for k, v in self.flattenDict(val).items() ]
   else:
    return [ (key, val) ]
  items = [ item for k, v in d.items() for item in expand(k, v)]
  return dict(items)

 # def writeCommentsCSV( self, c_list):
 ...
 #  flat_comments_list = []
 #  for c in c_list:
 #   flat_comments_list.append(self.flattenDict(c))
